...
export default function AddExamPage() {
  ...
  const [form, setForm] = useState({
    name: '', class: '', subject: '', date: '', startTime: '', endTime: '', totalMarks: '', passingMarks: '',
  });
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-gray-800">Add Exam</h3>
        <nav className="flex items-center gap-2 text-sm mt-1">
          ...
          <span className="text-gray-500">Add Exam</span>
        </nav>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <Field label="Exam Name">
            <input value={form.name} onChange={set('name')} type="text" placeholder="e.g. Mid Term" className={input} required />
          </Field>
          <Field label="Class">
          ...
          <Field label="Total Marks">
            <input value={form.totalMarks} onChange={set('totalMarks')} type="number" placeholder="e.g. 100" className={input} />
          </Field>
          <Field label="Passing Marks">
            <input value={form.passingMarks} onChange={set('passingMarks')} type="number" placeholder="e.g. 35" className={input} />
          </Field>
        </div>
