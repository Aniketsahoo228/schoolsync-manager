...
export default function AddTimetablePage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-gray-800">Add Time Table</h3>
        <nav className="flex items-center gap-2 text-sm mt-1">
          <Link href="/timetable" className="text-orange-500 hover:underline">Time Table</Link>
          <span className="text-gray-400">/</span>
          <span className="text-gray-500">Add Time Table</span>
        </nav>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <Field label="Teacher ID">
            <input value={form.teacherId} onChange={set('teacherId')} type="text" placeholder="e.g. PRE1534" className={input} required />
          </Field>
          <Field label="Teacher Name">
            <input value={form.name} onChange={set('name')} type="text" placeholder="e.g. John Smith" className={input} required />
          </Field>
          <Field label="Class">
        ...
        <div className="mt-8 flex gap-3">
          <button type="submit" disabled={loading} className={btn}>
            {loading ? 'Saving...' : 'Add Time Table'}
          </button>
          <button type="button" onClick={() => router.push('/timetable')} className={btnOutline}>
            Cancel
          </button>
        </div>
      </form>
